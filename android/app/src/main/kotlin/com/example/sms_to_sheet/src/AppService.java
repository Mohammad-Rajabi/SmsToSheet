...
public class AppService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        List<SmsModel> smsList = (List<SmsModel>) intent.getSerializableExtra(Constants.EXTRA_KEY_SMS_LIST);
        ...
        if (NetworkUtil.isInternetAvailable(getBaseContext())) {
            ...
            startForeground(Constants.NOTIFICATION_ID, NotificationUtil.getNotification(this, getResources().getString(R.string.sending_sms), R.drawable.ic_baseline_cloud_upload_24));

            for (int i = 0; i < smsList.size(); i++) {

                apiService.sendSms(smsList.get(i), new ResponseCallback() {
                    @Override
                    public void successful() {
                        NotificationUtil.updateNotification(AppService.this, getResources().getString(R.string.sent_sms_successful), R.drawable.ic_baseline_check_circle_24);
                    }
            ...
            startForeground(Constants.NOTIFICATION_ID, NotificationUtil.getNotification(this, getResources().getString(R.string.no_internet), R.drawable.ic_baseline_wifi_off_24));
